...
const Clothing = () => {
  ...
  useEffect(() => {
    if (assets.clothing && assets.clothing.length > 0) {
      setFilteredAssets(assets.clothing);
      
      // Extract available filters
      const tags = [...new Set(assets.clothing.flatMap(asset => asset.tags))];
      const creators = [...new Set(assets.clothing.map(asset => asset.creator))];
  ...
  useEffect(() => {
    if (!assets.clothing || assets.clothing.length === 0) return;
    
    let filtered = [...assets.clothing];
  ...
  }, [searchQuery, activeFilters, sortOption, assets.clothing]);
    ...
    <Box>
      ...
      >
        <Box sx={{ 
          ...
          <Typography variant="h1">Clothing Assets</Typography>
      ...
      {loading.assets ? (
        ...
        <Box sx={{ textAlign: 'center', py: 8 }}>
          <Typography variant="h2" sx={{ mb: 2 }}>No Clothing Found</Typography>
          <Typography variant="body1" color="text.secondary" sx={{ mb: 4 }}>
            {assets.clothing && assets.clothing.length === 0 
              ? "You haven't added any clothing assets yet." 
              : "No clothing matches your current filters."}
          </Typography>
          {assets.clothing && assets.clothing.length === 0 ? (
            <Button 
              variant="contained" 
