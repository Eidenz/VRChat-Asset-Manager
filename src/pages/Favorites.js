...
const Favorites = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      // Filter to only favorited assets
      const favoritedAssets = allAssets.filter(asset => asset.favorited);
      
      // Get favorited avatars
      const favoritedAvatars = avatars.filter(avatar => avatar.favorited);
      
      setAssets(favoritedAssets);
      setAvatarList(favoritedAvatars);
      setFilteredAssets(favoritedAssets);
      
      // Extract available filters
      const types = [...new Set(favoritedAssets.map(asset => asset.type))];
      const tags = [...new Set(favoritedAssets.flatMap(asset => asset.tags))];
  ...
  }, [searchQuery, activeFilters, sortOption, assets, avatarList, tabValue]);
      ...
      >
        <Box sx={{ 
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Typography>
              {tabValue === 'avatars' 
                ? `${avatarList.length} ${avatarList.length === 1 ? 'avatar' : 'avatars'}`
