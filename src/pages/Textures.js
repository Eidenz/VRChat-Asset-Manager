...
import { textureAssets } from '../data/mockData';
...
const Textures = () => {
  ...
  useEffect(() => {
    // Simulate API call
    setTimeout(() => {
      // Add a local favorite state to each asset
      const assetsWithLocalState = textureAssets.map(asset => ({
        ...asset,
        localFavorite: asset.favorited || false
      }));
      
      setAssets(assetsWithLocalState);
      setFilteredAssets(assetsWithLocalState);
      
      // Extract available filters
      const tags = [...new Set(textureAssets.flatMap(asset => asset.tags))];
      const creators = [...new Set(textureAssets.map(asset => asset.creator))];
  ...
  const handleToggleFavorite = (asset, e) => {
    if (e) e.stopPropagation();
    
    // Update the assets array with the toggled favorite state
    setAssets(prevAssets => 
      prevAssets.map(a => 
        a.id === asset.id ? { ...a, localFavorite: !a.localFavorite } : a
      )
    );
  };
                  ...
                  >
                    <Box sx={{ position: 'relative', width: 120 }}>
                      ...
                      <IconButton
                        ...
                        sx={{ 
                          ...
                          width: 28, 
                          height: 28,
                          backgroundColor: asset.localFavorite ? 'error.main' : 'rgba(0,0,0,0.5)',
                          color: '#fff',
                          '&:hover': {
                            backgroundColor: asset.localFavorite ? 'error.dark' : 'primary.main',
                          }
                        }}
                      >
                        {asset.localFavorite ? 
                    ...
                    <Box sx={{ 
                      ...
                      <Tooltip title="Download">
                        <IconButton onClick={(e) => handleDownload(asset, e)}>
                          <DownloadIcon />
                        </IconButton>
                      </Tooltip>
